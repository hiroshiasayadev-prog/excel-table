...
Usage::
    ...
    schema = SheetWriteSchema(rows=[
        [formatted_value_map, chart_config],
    ])
...
from .models.format import FormattedTable2D, ChartConfig, ColorScale
from .models.table import Table2D, Table1D
...
def write_sheet(
    ...
    Note:
        Chart rendering is not yet implemented; :class:`ChartConfig` items
        reserve the correct cell footprint but do not produce a chart.
    ...
    for row_items in schema.rows:
        ...
        for item in row_items:
            if isinstance(item, FormattedTable2D):
                ...
                # chart rendering delegated to chart.py – not yet implemented
                current_col += cols + TABLE_MARGIN_COLS
