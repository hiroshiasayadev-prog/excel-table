...
same list are also accepted. For example, if a subclass defines::
    ...
    @property
    def torque(self) -> list[str]:
        return self.row

then ``row_filter = "torque >= 0"`` is equivalent to ``row_filter = "row >= 0"``.
...
from .models.format import ChartConfig, LineSeriesConfig, ColorScale, FormattedTable2D
from .models.table import Table2D
from .writer import _colorscale_color, _hex
...
def _validate_filter_expr(
    ...
    Args:
        expr: Python expression string (e.g. ``"torque >= 0"``).
